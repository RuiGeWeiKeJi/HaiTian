...
namespace StringEncrypt
{
    public class Encrypt
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal bool registerYesOrNo ( )
        {
            bool result = false;
            result = SqlHelper.Exists( "SELECT COUNT(*) FROM master..sysobjects WHERE name='rgw_register'" );
            if ( result == true )
            {
                ...
                if ( dt != null && dt.Rows.Count > 0 )
                {
                    ...
                    {
                        //AE9A5556670E
                        //DataTable dk = SqlHelper.ExecuteDataTable( "select top 1 net_address from master..sysprocesses where hostname = left (@@servername ,charindex ( '\\' ,@@servername + '\\' ) - 1 )" );
                        //if ( dk != null && dk.Rows.Count > 0 )
                        //{
                        //    if ( !string.IsNullOrEmpty( dk.Rows[0]["net_address"].ToString( ) ) )
                        //    {
                        //        if ( new Encrypt( ).ToDecrypt( "8899" ,dt.Rows[0]["Regisnum"].ToString( ) ) == dk.Rows[0]["net_address"].ToString( ) )
                        //            result = true;
                        //        else
                        //            result = false;
                        //    }
                        ...
                        //else
                        //    result = false;
                        if ( new Encrypt ( ) . ToDecrypt ( "8899" ,dt . Rows [ 0 ] [ "Regisnum" ] . ToString ( ) ) == "AE9A5556670E" )
                            result = true;
                        else
                            result = false;
                    }
