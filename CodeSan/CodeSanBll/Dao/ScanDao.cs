...
namespace CodeSanBll . Dao
{
    public class ScanDao
    {
        ...
        public DataTable getDataTableOfOrder ( string LotId )
        {
            ...
            strSql . AppendFormat ( "SELECT DISTINCT IBB001 FROM HDTBAR A INNER JOIN DCSIBB B ON A.BAR001=B.IBB003 WHERE IBB015='N' AND IBB023='T' AND BAR012='T' AND BAR016!='T' AND BAR008='{0}' ORDER BY IBB001 DESC" , LotId );
        ...
        {
            StringBuilder strSql = new StringBuilder ( );
            strSql . AppendFormat ( "SELECT BAR008,BAR007,BAR004 FROM HDTBAR WHERE BAR018='{0}'" , code );
        ...
        {
            StringBuilder strSql = new StringBuilder ( );
            strSql . AppendFormat ( "SELECT BAR001,BAR004,BAR007,BAR008,ISNULL(BAR020,0) BAR020 FROM HDTBAR WHERE BAR008='{0}' AND BAR012='T' AND BAR016!='T'" , lotId );
        ...
        /// <returns></returns>
        string IsertJSKLIA ( CodeSanEntity . JSKLIA _modelTre , StringBuilder strSql )
        {
        ...
        /// <param name="strSql"></param>
        /// <returns></returns>
        string EditHDTBAR ( StringBuilder strSql , CodeSanEntity . StoEntity _model )
        {
            strSql = new StringBuilder ( );
        ...
        {
            DateTime dt = DicSystem . getTimeNow ( );
            StringBuilder strSql = new StringBuilder ( );
            strSql . Append ( "SELECT MAX(LIA001) LIA001 FROM JSKLIA " );
            strSql . Append ( "WHERE LIA001 LIKE '" + dt . ToString ( "yyyyMMdd" ) + "%'" );
            ...
            if ( da != null && da . Rows . Count > 0 )
            {
                ...
                if ( string . IsNullOrEmpty ( strOdd ) )
                    strOdd = dt . ToString ( "yyyyMMdd" ) + "001";
                else
        ...
        void KEAKEB ( DataTable dt , ArrayList SQLString , StringBuilder strSql , CodeSanEntity . JSKKEA _modelOne , CodeSanEntity . JSKKEB _modelTwo , string personNum , string orderType , CodeSanEntity . StoEntity _model )
        {
            try
            {
                SQLString . Clear ( );
                ...
                for ( int i = 0 ; i < dt . Rows . Count ; i++ )
                {
                    ...
                    _model . BAR008 = dt . Rows[i]["BAR008"] . ToString ( );
                    _model . BAR020 = string . IsNullOrEmpty ( dt . Rows[i]["BAR020"] . ToString ( ) ) == true ? 0 : Convert . ToInt32 ( dt . Rows[i]["BAR020"] . ToString ( ) );
                    num = Convert . ToInt32 ( _model . BAR020 );
                    _model . BAR020 += getCkNum ( _model . BAR008 );
                    _modelOne . KEA002 = "条码";
        ...
        /// <param name="strSql"></param>
        /// <returns></returns>
        string InsertJSKKEA ( CodeSanEntity . JSKKEA _modelOne , StringBuilder strSql )
        {
            strSql = new StringBuilder ( );
        ...
        /// <param name="strSql"></param>
        /// <returns></returns>
        string IsertJSKKEB ( CodeSanEntity . JSKKEB _modelTwo , StringBuilder strSql )
        {
            strSql = new StringBuilder ( );
        ...
        {
            StringBuilder strSql = new StringBuilder ( );
            strSql . AppendFormat ( "SELECT BAR001,BAR002,BAR003,BAR004,BAR007,BAR020,BAR019,BAR018 FROM HDTBAR WHERE {0}" , strWhere );
