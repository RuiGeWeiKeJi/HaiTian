...
namespace SmartDeviceProjectBll.Dao
{
    public class ConnecDao
    {
        ...
        /// <param name="lotId"></param>
        /// <returns></returns>
        public string lotidDt(string lotId, string spe)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("SELECT DISTINCT BAR008 FROM HDTBAR WHERE BAR007='{0}' AND BAR004='{1}'  AND ISNULL(BAR020,0)<ISNULL(BAR019,0) ", lotId, spe);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("SELECT DISTINCT BAR004 FROM HDTBAR WHERE BAR007='{0}'AND ISNULL(BAR020,0)<ISNULL(BAR019,0) ", lotId);
